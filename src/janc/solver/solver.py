...
def CFL(field,cfl=0.20):
    U, aux = field[0:-2],field[-2:]
    _,u,v,w,_,_,a = aux_func.U_to_prim(U,aux)
    cx = jnp.max(abs(u) + a)
    cy = jnp.max(abs(v) + a)
    cz = jnp.max(abs(w) + a)
    dt = jnp.minimum(cfl*gd.dxi/cx,cfl*gd.deta/cy,cfl*gd.ddelta/cz)
    return dt
...
def set_solver(thermo_set, boundary_set, source_set = None, nondim_set = None):
    ...
    def rhs(U,aux,theta=None):
        aux = aux_func.update_aux(U, aux)
        U_with_ghost,aux_with_ghost = boundary.boundary_conditions(U,aux,theta)
        physical_rhs = weno5(U_with_ghost,aux_with_ghost) + aux_func.source_terms(U, aux, theta)
        return physical_rhs
    ...
    def advance_flux(field,dt,theta=None):
        ...
        U, aux = field[0:-2],field[-2:]
        U1 = U + dt * rhs(U,aux,theta)
        U2 = 3/4*U + 1/4 * (U1 + dt * rhs(U1,aux,theta))
        U3 = 1/3*U + 2/3 * (U2 + dt * rhs(U2,aux,theta))
    ...
    def advance_one_step(field,dt,theta=None):
        field_adv = advance_flux(field,dt,theta)
