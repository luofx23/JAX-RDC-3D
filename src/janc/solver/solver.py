...
def set_solver(thermo_set, boundary_set, source_set = None, nondim_set = None, solver_mode='base'):
    ...
    if solver_mode == 'amr':
        ...
        def rhs(U, aux, dx, dy, dz, theta=None):
            ...
            physical_rhs = weno5(U,aux,dx,dy,dz) + aux_func.source_terms(U[:,3:-3,3:-3,3:-3], aux[:,3:-3,3:-3,3:-3], theta)
            return jnp.pad(physical_rhs,pad_width=((0,0),(3,3),(3,3),(3,3)))
    ...
    if solver_mode == 'amr':
        def advance_flux(level, blk_data, dx, dy, dz, dt, ref_blk_data, ref_blk_info, theta=None):
            ...
            blk_data1 = amr.update_external_boundary(level, blk_data, blk_data1[..., num:-num, num:-num, num:-num], ref_blk_info)
            ...
            blk_data2 = amr.update_external_boundary(level, blk_data, blk_data2[..., num:-num, num:-num, num:-num], ref_blk_info)
            ...
            blk_data3 = amr.update_external_boundary(level, blk_data, blk_data3[..., num:-num, num:-num, num:-num], ref_blk_info)
            
            return blk_data3
    
    else:
        def advance_flux(field,dx,dy,dz,dt,theta=None):
    ...
    if chem_solver_type == 'implicit':
        if solver_mode == 'amr':
            ...
            def advance_one_step(level, blk_data, dx, dy, dz, dt, ref_blk_data, ref_blk_info,theta=None):
                field_adv = advance_flux(level, blk_data, dx, dy, dz, dt, ref_blk_data, ref_blk_info,theta)
            ...
            if solver_mode == 'parallel':
                ...
                @jit    
                def advance_one_step(field,dx,dy,dz,dt,theta=None):
                    field_adv = advance_flux(field,dx,dy,dz,dt,theta)
        ...
        if solver_mode == 'amr':
            @partial(jit,static_argnames='level')
            def advance_one_step(level, blk_data, dx, dy, dz,dt, ref_blk_data, ref_blk_info,theta=None):
                field = advance_flux(level, blk_data, dx, dy, dz,dt, ref_blk_data, ref_blk_info, theta)
                return field
            ...
            if solver_mode == 'parallel':
                ...
                @jit    
                def advance_one_step(field,dx,dy,dz,dt,theta=None):
                    field = advance_flux(field,dx,dy,dz,dt,theta)
                    return field
            else:
                @jit    
                def advance_one_step(field,dx,dy,dz,dt,theta=None):
