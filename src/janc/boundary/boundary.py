...
from .boundary_padding import pad, replace_lb, replace_rb, replace_ub, replace_bb,replace_fb,replace_backb
...
def set_boundary(boundary_config:dict):
    ...
    if callable(boundary_config['front_boundary']):
        def front_boundary(padded_U,padded_aux,theta=None):
            U_ub,aux_ub = padded_U[:,3:-3,3:-3,3:6],padded_aux[:,3:-3,3:-3,3:6]
            U_ub,aux_ub = boundary_config['front_boundary'](U_ub,aux_ub,theta)
            U_with_ub,aux_with_ub = replace_fb(U_ub,aux_ub,padded_U,padded_aux)
            return U_with_ub,aux_with_ub
    else:
        assert (boundary_config['front_boundary'] == 'slip_wall') or\
               (boundary_config['front_boundary'] == 'periodic') or\
                   (boundary_config['front_boundary'] == 'neumann') or\
               (boundary_config['front_boundary'] == 'pressure_outlet'),\
                'the bc type is not supported, please try custom  bc.'
                
        if boundary_config['front_boundary'] == 'slip_wall':
            def front_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,3:6],padded_aux[:,3:-3,3:-3,3:6]
                U_ub,aux_ub = slip_wall.front(U_ub,aux_ub)
                U_with_ub,aux_with_ub = replace_fb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
        elif boundary_config['front_boundary'] == 'periodic':
            def front_boundary(padded_U,padded_aux,theta=None):
                return padded_U,padded_aux
        elif boundary_config['front_boundary'] == 'neumann':
            def front_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,3:6],padded_aux[:,3:-3,3:-3,3:6]
                U_ub,aux_ub = neumann.front(U_ub,aux_ub)
                U_with_ub,aux_with_ub = replace_fb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
        elif boundary_config['front_boundary'] == 'pressure_outlet':
            def front_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,3:6],padded_aux[:,3:-3,3:-3,3:6]
                U_ub,aux_ub = pressure_outlet.front(U_ub,aux_ub,theta)
                U_with_ub,aux_with_ub = replace_fb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
    ...
    if callable(boundary_config['back_boundary']):
        ...
        assert (boundary_config['back_boundary'] == 'slip_wall') or\
               (boundary_config['back_boundary'] == 'periodic') or\
                   (boundary_config['back_boundary'] == 'neumann') or\
               (boundary_config['back_boundary'] == 'pressure_outlet'),\
                'the bc type is not supported, please try custom  bc.'
                
        if boundary_config['back_boundary'] == 'slip_wall':
            def back_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,-6:-3],padded_aux[:,3:-3,3:-3,-6:-3]
                U_ub,aux_ub = slip_wall.back(U_ub,aux_ub)
                U_with_ub,aux_with_ub = replace_backb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
        elif boundary_config['back_boundary'] == 'periodic':
            def back_boundary(padded_U,padded_aux,theta=None):
                return padded_U,padded_aux
        elif boundary_config['back_boundary'] == 'neumann':
            def back_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,-6:-3],padded_aux[:,3:-3,3:-3,-6:-3]
                U_ub,aux_ub = neumann.back(U_ub,aux_ub)
                U_with_ub,aux_with_ub = replace_backb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
        elif boundary_config['back_boundary'] == 'pressure_outlet':
            def back_boundary(padded_U,padded_aux,theta=None):
                U_ub,aux_ub = padded_U[:,3:-3,3:-3,-6:-3],padded_aux[:,3:-3,3:-3,-6:-3]
                U_ub,aux_ub = pressure_outlet.back(U_ub,aux_ub,theta)
                U_with_ub,aux_with_ub = replace_backb(U_ub,aux_ub,padded_U,padded_aux)
                return U_with_ub,aux_with_ub
...
def boundary_conditions(U, aux, theta=None):
    ...
    U_with_ub,aux_with_ub = boundary_func['up_boundary'](U_with_bb,aux_with_bb,theta)
    U_with_fb,aux_with_fb = boundary_func['front_boundary'](U_with_ub,aux_with_ub,theta)
    U_with_ghost_cell,aux_with_ghost_cell = boundary_func['back_boundary'](U_with_fb,aux_with_fb,theta)
