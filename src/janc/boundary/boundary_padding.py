...
def pad(U,aux):
    ...
    field_periodic_y = jnp.concatenate([field_periodic_x[:,:,-4:-3,:],field_periodic_x[:,:,-3:-2,:],field_periodic_x[:,:,-2:-1,:],field_periodic_x,field_periodic_x[:,:,1:2,:],field_periodic_x[:,:,2:3,:],field_periodic_x[:,:,3:4,:]],axis=2)
    field_periodic_pad = jnp.concatenate([field_periodic_y[:,:,:,-4:-3],field_periodic_y[:,:,:,-3:-2],field_periodic_y[:,:,:,-2:-1],field_periodic_y,field_periodic_y[:,:,:,1:2],field_periodic_y[:,:,:,2:3],field_periodic_y[:,:,:,3:4]],axis=3)
...
def replace_lb(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,0:3,3:-3,3:-3].set(U_bd)
    aux = padded_aux.at[:,0:3,3:-3,3:-3].set(aux_bd)
    return U, aux

    
def replace_rb(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,-3:,3:-3,3:-3].set(U_bd)
    aux = padded_aux.at[:,-3:,3:-3,3:-3].set(aux_bd)
    return U, aux
...
def replace_bb(U_bd, aux_bd, padded_U, padded_aux):  
    U = padded_U.at[:,3:-3,0:3,3:-3].set(U_bd)
    aux = padded_aux.at[:,3:-3,0:3,3:-3].set(aux_bd)
    return U, aux

def replace_fb(U_bd, aux_bd, padded_U, padded_aux):  
    U = padded_U.at[:,3:-3,3:-3,0:3].set(U_bd)
    aux = padded_aux.at[:,3:-3,3:-3,0:3].set(aux_bd)
    return U, aux

def replace_backb(U_bd, aux_bd, padded_U, padded_aux):
    U = padded_U.at[:,3:-3,3:-3,-3:].set(U_bd)
    aux = padded_aux.at[:,3:-3,3:-3,-3:].set(aux_bd)
    return U, aux
